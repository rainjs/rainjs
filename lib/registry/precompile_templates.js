...
function configure(componentConfig) {
    ...
    var default_language = conf.default_language;
    ...
    for (var viewId in componentConfig.views) {
        ...
        if(!filePath) {
            filePath = getFilePath(templatesFolder, basename, default_language);
        }
        if(!filePath) {
 ...
 * @param {String} basename Basename of the template ( without .html )
 * @param {String} language The language
 * @returns {String} filePath or null
 */
function getFilePath(templatesFolder, basename, language){
    var filePath = path.join(templatesFolder, basename + '_' + language + '.html');
    if(language == 'en_US'){
