...
ComponentHelper.prototype.helper = function (options) {
    ...
    if (!version) {
        var tagLib = exceptionHandler.renderException(404, new Error('Component ' + component + ' not found!'));
        ...
        if (!requiredView) {
            var tagLib = exceptionHandler.renderException(404, new Error('Component ' + configuration.id + ' does not contain the view ' + view));
            rainComponent.addTagLib(tagLib);

            return renderHtml(tagLib.selector);
        }
        
        var userPermissions = rain.session.user && rain.session.user.permissions ? rain.session.user.permissions : [];
        var requiredPermissions = [];
        //add component permissions
        if (configuration.permissions) {
            requiredPermissions = requiredPermissions.concat(configuration.permissions);
        }
        //add view permissions
        if (requiredView.permissions) {
            requiredPermissions = requiredPermissions.concat(requiredView.permissions);
        }
        
        if (mod_authorization.authorize(userPermissions, requiredPermissions) === false) {
            
            var tagLib = exceptionHandler.renderException(401, new Error('Unauthorized access to component "' + component + '" !'));
            rainComponent.addTagLib(tagLib);
