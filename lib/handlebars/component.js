...
ComponentHelper.prototype.helper = function (options) {
    ...
    if (!componentId) {
        ...
        if (!version) {
            var error = errorHandler.getErrorComponent(404, { message: 'Component ' + componentId + ' not found!', type: 'templating' });
            componentId = error.component.id;
            viewId = error.view.view;
        }
    }
    ...
    rain.childrenInstanceIds.push(instanceId);
    ...
    dataLayer.loadData({
        ...
        renderer.sendComponent(rain.transport, {
            component: componentRegistry.getConfig(componentId, version),
            viewId: viewId,
            staticId: staticId,
