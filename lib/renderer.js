...
function registerWebsocketLayer(self){
    socketRegistry.register('/core', function (socket) {
        socket.on('render', function(component, acknowledge){
            ...
            var componentConfig = componentRegistry.getConfig(componentId, version);
            ...
            dataLayer.loadData({
                ...
                if (err instanceof Error) {
                    var error = errorHandler.getErrorComponent(500);
                    component.componentId = error.component.id;
                    component.version = error.component.version;
                    component.view = error.view;
                    err.stack = err.stack.replace(/ /g, '&nbsp;').replace(/\n/g, '<br />');
                    component.data = { error: err };

                    templateData = component.data;
                    componentConfig = componentRegistry.getConfig(component.componentId,
                                                                  component.version);
                }
                if (!templateData) {
                    templateData = {};
...
Renderer.prototype.createRainContext = function(opt) {
    ...
    util.extendObject(settings, opt);
    return settings;
};
