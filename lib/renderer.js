...
function registerWebsocketLayer(self){
    socketRegistry.register('/core', function (socket) {
        socket.on('render', function(component, acknowledge){
            ...
            if(!self.isAuthorized(component)){
...
/**
 * Checks the user permissions and replace the component with an error component if the user ha no rights
 *
 * @param {Object} childComponent The component information for the rendering process
 * @return {Boolean} Returns the authorization status
 */
Renderer.prototype.isAuthorized = function(childComponent){
    ...
    var securityContext = createSecurityContext({
        user: childComponent.session && childComponent.session.user
    });
