...
Renderer.prototype.renderComponent = function (opt) {
    ...
    if (!view) {
        ...
        return this.renderComponent({
            ...
            context: { error: new RainError("Can't render component! View: %s doesn't exists!", [viewId]) },
...
function registerWebsocketLayer(self) {
    socketRegistry.register('/core', function (socket) {
        socket.on('render', function (component, acknowledge) {
            ...
            if(!self.isValidView(component)){
                self.loadDataAndSend(component, socket);
            }

            if (!self.isAuthorized(component, self.AUTHORIZATION_TYPE_VIEW)) {
                self.replaceWithError(401, component,
...
Renderer.prototype.loadDataAndSend = function (component, transport) {
    ...
    dataLayer.loadData({
        ...
        if (err instanceof Error) {
            self.replaceWithError(500, component, err);
 ...
 * @memberOf Renderer#
 */
Renderer.prototype.isValidView = function(component){
    var rain = this.rain;
    if (!component.view) {
        this.replaceWithError(500, component,
    ...
    if (!component.id) {
        component.id = rain.component.id;
        component.version = rain.component.version;
    } else {
        component.version = componentRegistry.getLatestVersion(component.id,
                                                                    component.version);
        if (!component.version) {
            this.replaceWithError(404, component,
 ...
 */
Renderer.prototype.isAuthorized = function (component, type) {
    var componentConfig = componentRegistry.getConfig(component.id,
                                                      component.version);
