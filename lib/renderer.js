...
Renderer.prototype.renderBootstrap = function(mainComponent, viewId, request, response) {
    ...
    /**
     * if mainComponent is undefined then the requested component dosn't exist
     */
    if(!mainComponent){
        var error = errorHandler.getErrorComponent(404);
        mainComponent = error.component;
        viewId = error.view;
    }
...
Renderer.prototype.renderComponent = function(opt) {
    ...
    if (!view) {
        ...
        return this.renderComponent({
            ...
            data: { error: new RainError("Can't render component! View: " + viewId + " doesn't exists!") },
 ...
 * @returns {String}
 */
Renderer.prototype.createInstanceId = function (pInstanceId, staticId) {
    return crypto.createHash('sha1').update(((new Date()).getTime()+staticId+pInstanceId).toString()).digest('hex');
};
