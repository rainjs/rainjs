        ...
        case 'view':
            if (!component.views[intent.view]) {
                throw new RainError('View %s was not found in %s;%s.',
                                    [intent.view, component.id, component.version]);
            }
        ...
        switch (handler.type) {
            ...
            case 'server':
                ...
                try {
                    ...
                    acknowledge('Internal intent error for component ' + component.id + '!');
                    return;
                }
                break;
        }
...
function resolveIntent(category, action) {
    ...
    if (!categories) {
        return 'Intent category ' + category + ' was not found.';
    }

    var handlers = categories[action];
    if (!handlers) {
        return 'Intent action ' + action + ' was not found.';
    }
