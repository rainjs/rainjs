...
function Monitoring () {
    ...
    setInterval(function () {self._composeDataAndSend();}, this._defaultSend*100);
}
...
Monitoring.prototype._composeDataAndSend = function () {
    ...
    for (i in this._compositionMap) {
        for(j in this._compositionMap[i].measurements) {
            if (this._compositionMap[i].op) {
                var value =
                    this._compositionMap[i].measurements[j].total / this._compostionMap[i].measurements[j].requests;
            } else {
                value = this._compositionMap[i].measurments[j].total;
            }
        }

        composition.push({
            host: this._defaultHost,
            key: this._compositionMap[i].key,
            value: value
        });
