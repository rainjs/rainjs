...
function registerPlugins() {
    for (var i = plugins.length; i--;) {
        try {
            ...
            throw new RainError('Registry Plugin ' + file + 'is invalid.', RainError.ERR_IO);
        }
    }
}
...
function scanComponentFolder() {
    ...
    try {
        ...
        throw new RainError('The components folder does not exist!', ex, RainError.ERROR_IO);
    }
...
function configurePlugins() {
    for (var component in componentMap) {
        var versions = componentMap[component];
        for (var version in versions.config) {
            for (var j = plugins.length; j--;) {
                plugins[j].configure(versions.config[version]);
            }
        }
    }
}
