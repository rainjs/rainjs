 ...
 */
ComponentRegistry.prototype.initialize = function (config, plugins) {
    if(!config){
        throw new RainError("An instance of configuration is required!");
    }
...
function registerPlugins() {
    for (var i = registeredPlugins.length; i--;) {
        try {
            var plugin = require(path.join(ComponentRegistry.PLUGIN_FOLDER, registeredPlugins[i]));

            registeredPlugins[i] = plugin;
            console.info("Loaded registry plugin: " + plugin.name);
        } catch (ex) {
            throw new RainError('Registry Plugin %s is invalid.', [registeredPlugins[i]], RainError.ERR_IO);
        }
    }
}

/**
 * Scans the component folder and register the components.
 *
...
function scanComponentFolder() {
    ...
    try {
        ...
        throw new RainError('The components folder does not exist!', RainError.ERROR_IO);
    }
    ...
    for (var i = 0, len = folders.length; i < len; i++) {
        ...
        try {
            ...
            registerComponent(config, folder);
        } catch (ex) {
            console.log('Failed to load meta.json for %s component!', folder);
        }
    }
}
...
function configurePlugins() {
    for (var component in componentMap) {
        ...
        for (var version in versions) {
            for (var j = registeredPlugins.length; j--;) {
                registeredPlugins[j].configure(versions[version]);
            }
        }
    }
}
...
function registerComponent(config, folder) {
    ...
    console.debug("Register component: ", config.id);
